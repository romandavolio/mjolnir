...
class _BodyWeightScreenState extends State<BodyWeightScreen> {
  ...
  Future<void> _loadHistory() async {
    final history = await BodyWeightService.loadHistory(uid: widget.viewAsUid);
    if (!mounted) return;
    setState(() {
      _history = history;
      _loading = false;
              ...
              children: [
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    // Actualizar peso en el perfil
                    final profile = await UserService.getCurrentProfile();
                    if (profile != null) {
                      await AuthService.updateProfile(
                        profile.copyWith(weight: selectedWeight),
                      );
                    }
